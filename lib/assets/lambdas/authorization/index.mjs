...
const apiRegion = process.env.APIABLE_AWS_AUTHZ_API_GATEWAY_REGION
...
export const handler = async (event, context, callback) => {
    ...
    if (!iamPolicyResult && apiKey) {
        try {
            if (isLikelyJWT(apiKey)) {
                throw new Error("API key appears to be a JWT");
            }
            iamPolicyResult = await assumeAwsApiKey(event, apiKey);
        } catch (e) {
