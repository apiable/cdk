...
function getServiceArn(resources, methodArn) {
    ...
    if (!!resources) {
        ...
        resourcePolicies = resources
            ...
            .replace(/{[^}]+}/g, '')
            .split(';');
        resourcePolicies.split(';')
        for (let i = 0; i < resourcePolicies.length; i++) {
            if (resourcePolicies[i] && resourcePolicies[i].length > 0 && resourcePolicies[i].slice(-1) === '/') {
                resourcePoliciesFiltered = resourcePolicies[i].slice(0, -1); // Remove the last character
            } else {
                resourcePoliciesFiltered = resourcePolicies[i]
            }
        }
        console.log('after replace:', resourcePoliciesFiltered.join(';'))
    ...
    if (parts.length === 6) {
        ...
        if (pathParts.length >= 4) {
            if (resourcePoliciesFiltered.length > 0) {
                for (let i = 0; i < resourcePoliciesFiltered.length; i++) {
                    parts[5] = resourcePoliciesFiltered[i];
...
const assumeAWSToken = async (event, token) => {
    try {
        ...
        */
        ...
        console.log("Token is not AWS Cognito Token")
        return null;
    }
}
...
const assumeAwsApiKey = async (event, token) => {
    try {
        ...
        console.log("Token is AWS Api Gateway Key")
        const planResources = ""
