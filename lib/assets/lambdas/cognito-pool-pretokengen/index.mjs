...
//   apiable_api_key        -> the authorizer returns it as usageIdentifierKey, so the consumer never
//                             sends x-api-key (the hidden-key, single-credential path).
...
//
// apiable_plan_resources has no machine-to-machine source on this path (no user => no user-attribute
// pipeline), so it ships empty and the per-method resource binding is deferred to a dedicated binding
// story. The live entitlement is the native `scope` claim. See the AWS V3_0 event/response shape:
